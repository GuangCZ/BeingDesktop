...
const {orchestrationInstructions}=require('./orchestration-message.cjs');

// Runs before Loom's own scripts, including its history/stream prefetches.
function installSessions(requestedId = null, routingFactory = createSessionRouter, messageContext = '', orchestration = {enabled:false}, buildOrchestrationInstructions = orchestrationInstructions) {
  ...
  globalThis.fetch = async function(input, options) {
    ...
    if (chat) {
      ...
      const instructions=buildOrchestrationInstructions(mode);
