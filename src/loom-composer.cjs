...
function normalizeComposerData(value = {}) {
  ...
  const normalize = (items, kind) => {
    ...
    for (const item of Array.isArray(items) ? items.slice(0, 1000) : []) {
      ...
      result.push({id:item.id, name, handle, description:clean(item.description || item.bio, 220), kind, installed:item.installed === true, icon:kind === 'kit' ? bundledKitIcon(item.id) : '', builtin:kind === 'kit' && BUILTIN_KITS.includes(item) ? item.builtin : ''});
    }
    return result;
  };
  return {kits:normalize([...BUILTIN_KITS, ...(Array.isArray(value.kits) ? value.kits.slice(0,1000) : [])], 'kit'), members:normalize(value.members, 'member'), kitsError:clean(value.kitsError, 200), membersError:clean(value.membersError, 200)};
}
