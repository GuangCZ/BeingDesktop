...
window.beingPortalUpdates = (() => {
  ...
  function render() {
    ...
    $('portal-update-version').textContent = current || (update.status === 'not_installed' ? '尚未安装' : '未识别');
    $('portal-update-check').disabled = checking || typeof bridge?.checkPortalUpdates !== 'function';
    $('portal-update-check').textContent = checking ? '检查中…' : failed ? '重试检查' : '检查更新';
    ...
    let message = '';
    if (available) message = `发现 Portal 新版本${latest ? ` ${latest}` : ''}，可查看更新。`;
    else if (checking) message = '正在检查 Portal 更新…';
    else if (failed) message = '暂时无法检查 Portal 更新，请重试。';
    else if (update.status === 'not_installed') message = '安装 Portal 后，会自动检查新版本。';
    ...
    $('portal-update-checked').textContent = checkedAt && !Number.isNaN(checkedAt.getTime())
      ? `上次检查 ${checkedAt.toLocaleString('zh-CN', {month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false})}` : '';
