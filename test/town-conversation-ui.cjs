...
async function run() {
  ...
  await execute(`(()=>{const value=fixture.envelope();value.snapshot.source='being_relay';fixture.publish(value);})()`); await settle();
  await check('Being-relayed messages visibly retain their unverified provenance', `document.querySelector('.ta-bonfire').textContent.includes('Being 转交 · 原文未独立核验')&&document.querySelectorAll('.ta-bonfire .ta-message').length>0`);
  await execute(`fixture.publish(fixture.envelope())`); await settle();
  await check('a later native snapshot removes the relay label', `!document.querySelector('.ta-bonfire').textContent.includes('Being 转交 · 原文未独立核验')`);
