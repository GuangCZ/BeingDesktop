...
if(!process.versions.electron) {
  ...
  let mode={mode:{enabled:false,defaultAgent:'codex',paths:{}},agents:[{id:'codex',name:'Codex CLI',status:'ready',detail:'执行接口与本机登录状态已确认。'},{id:'claude',name:'Claude Code CLI',status:'needs_auth',detail:'请先在终端完成 claude auth login，再重新检测。'},{id:'cursor',name:'Cursor CLI',status:'missing',detail:'未找到可执行程序。'},{id:'grok',name:'Grok Build CLI',status:'missing',detail:'未找到可执行程序。'}],workers:[worker]};
  ...
  async function run() {
    ...
    handle('saveOrchestration',value=>{modeSaves.push(structuredClone(value));if(rejectModeSave)throw new Error('本机 Worker 工具不可用');mode={...mode,mode:value};return mode;});
    ...
    assert.equal(await execute('document.querySelector(".session-workers").previousElementSibling.querySelector(".session-title").textContent'),'会话 1');
    // Worker events arrive independently of the shell's full state snapshot.
    // Exercise them without a page change or a second, unrelated refresh.
    mode={...mode,workers:[]};
    win.webContents.send('being:workers',mode);
    await waitFor('document.querySelectorAll(".session-workers").length===0');
    worker.status='queued';
    mode={...mode,mode:{...mode.mode,enabled:true},workers:[worker]};
    win.webContents.send('being:workers',mode);
    await waitFor('document.querySelector(".worker-shortcut")?.dataset.status==="queued"');
    assert.equal(await execute('document.querySelector(".worker-caption").textContent'),'codex · 排队中');
    await execute('document.querySelector(".worker-group-toggle").click()');
    worker.status='running';win.webContents.send('being:workers',mode);
    await waitFor('document.querySelector(".worker-shortcut")?.dataset.status==="running"');
    assert.equal(await execute('document.querySelector(".worker-caption").textContent'),'codex · 执行中');
    assert.equal(await execute('document.querySelector(".worker-group-toggle").getAttribute("aria-expanded")'),'false');
    assert.equal(await execute('document.querySelector(".worker-group-list").hidden'),true);
    worker.status='completed';win.webContents.send('being:workers',mode);
    await waitFor('document.querySelector(".worker-shortcut")?.dataset.status==="completed"');
    assert.equal(await execute('document.querySelector(".worker-group-summary").textContent'),'1 个 Worker · 0 执行中');
    await execute('document.querySelector(".worker-group-toggle").click()');
    // A full shell update must use the new workers before the sidebar caches it.
    mode={...mode,workers:[]};state.orchestration=mode;win.webContents.send('being:state',state);
    await waitFor('document.querySelectorAll(".session-workers").length===0');
    worker.status='running';mode={...mode,mode:{...mode.mode,enabled:false},workers:[worker]};state.orchestration=mode;
    win.webContents.send('being:state',state);
    await waitFor('document.querySelector(".worker-shortcut")?.dataset.status==="running"');
    assert.equal(await execute('document.querySelector(".session-workers").previousElementSibling.dataset.taskId'),sessionOne);
    ...
    await execute('[...document.querySelectorAll("#sidebar-menu button")].find(node=>node.textContent==="重命名").click()');
    ...
    assert.equal(await execute('document.getElementById("sidebar-scroll").scrollHeight>document.getElementById("sidebar-scroll").clientHeight'),true);
    ...
    assert.equal(await execute('(()=>{const w=document.querySelector(".worker-shortcut").getBoundingClientRect(),p=document.getElementById("sidebar-scroll").getBoundingClientRect(),b=document.querySelector(".sidebar-bottom").getBoundingClientRect();return w.top>=p.top&&w.bottom<=p.bottom+1&&w.bottom<=b.top;})()'),true);
    ...
    assert.equal(mode.mode.enabled,true);
    assert.equal(modeSaves.length,2,'One save per toggle, including the rejected attempt');
    assert.equal(await execute('document.getElementById("orchestration-save")===null&&document.getElementById("orchestration-retry").hidden'),true);
    await execute('document.getElementById("orchestration-default").value="claude";document.getElementById("orchestration-default").dispatchEvent(new Event("change",{bubbles:true}))');
    await waitFor('document.getElementById("orchestration-enabled").disabled===false');
    assert.equal(mode.mode.defaultAgent,'claude');assert.equal(modeSaves.length,3);
    await execute('document.getElementById("agent-path-claude").value="/fixture/claude";document.getElementById("agent-path-claude").dispatchEvent(new Event("input",{bubbles:true}));document.getElementById("agent-path-claude").dispatchEvent(new Event("change",{bubbles:true}))');
    await waitFor('(async()=>{const value=await window.beingDesktop.getOrchestration();return value.mode.paths.claude==="/fixture/claude"&&!document.getElementById("orchestration-enabled").disabled})()');
    assert.equal(mode.mode.paths.claude,'/fixture/claude');assert.equal(modeSaves.length,4);
    mode={...mode,enforcement:{status:'enforced',detail:'已核验当前 Desktop：本机执行通过 Worker 调度。'}};win.webContents.send('being:workers',mode);
    await waitFor('document.getElementById("orchestration-status").textContent.includes("已就绪")');
    assert.equal(await execute('document.getElementById("orchestration-policy-status").textContent.includes("已核验")'),true);
    rejectModeSave=true;
    await execute('document.getElementById("orchestration-default").value="codex";document.getElementById("orchestration-default").dispatchEvent(new Event("change",{bubbles:true}))');
    await waitFor('!document.getElementById("orchestration-retry").hidden&&!document.getElementById("orchestration-retry").disabled');
    assert.equal(mode.mode.defaultAgent,'claude');rejectModeSave=false;
    await execute('document.getElementById("orchestration-retry").click()');
    await waitFor('document.getElementById("orchestration-retry").hidden');assert.equal(mode.mode.defaultAgent,'codex');assert.equal(modeSaves.length,6);
    await execute('document.getElementById("orchestration-enabled").click()');
    await waitFor('document.getElementById("orchestration-status").textContent.includes("已关闭")&&!document.getElementById("orchestration-enabled").disabled');assert.equal(mode.mode.enabled,false);assert.equal(modeSaves.length,7);
    await execute('document.getElementById("orchestration-enabled").click()');
    await waitFor('document.getElementById("orchestration-status").textContent.includes("已就绪")&&!document.getElementById("orchestration-enabled").disabled');assert.equal(mode.mode.enabled,true);assert.equal(modeSaves.length,8);
    await capture('settings');
    // Every kit has a path row and a state line; the default picker lists the same four, Claude Code included.
    assert.deepEqual(await execute('[...document.querySelectorAll("#orchestration-default option")].map(option=>option.value)'),['codex','claude','cursor','grok']);
    assert.equal(await execute('document.getElementById("agent-state-claude").textContent'),'需要登录 · 请先在终端完成 claude auth login，再重新检测。');
    assert.equal(await execute('document.getElementById("agent-path-claude").placeholder'),'自动从 PATH 检测，或填写程序绝对路径');
    await execute('document.querySelector(".worker-shortcut").click()');
    ...
    await fs.writeFile(path.join(output,'report.json'),JSON.stringify({passed:true,checks:['session ownership','worker-only updates','new Worker appearance','live status changes','collapsed group state','completed Worker count','full snapshot ordering','worker detail navigation','tool event display','cancel worker','toggle auto-configuration','default Worker autosave','path autosave','configuration failure retry','live enforcement status','no manual save button','no horizontal overflow'],screenshots:['sessions.png','worker-details.png','settings.png']},null,2));
