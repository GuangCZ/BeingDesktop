...
const MODULE_SCRIPTS = Object.freeze({
  snapshot: `(() => {
    ...
    return {id:root?.dataset.townModule,root:geometry(root),
      ...
      portal:{workspaceMissing:workspace==='尚未选择文件夹',permissionsLink:visible(document.getElementById('portal-app-permissions')),noStalePlan:!portal?.textContent.includes('计划关闭'),sandboxLimitVisible:portal?.textContent.includes('不是操作系统沙箱'),confirmVisible:visible(portal?.querySelector('.ta-inline-confirm'))},
...
async function runTownUiAudit({ win, execute, observe, settle, check, report }) {
  ...
  for (const id of APP_IDS) {
    ...
    if(id==='portal') {
      check('town.portal-links-to-current-permissions',module.portal.permissionsLink&&module.portal.sandboxLimitVisible&&module.portal.noStalePlan,module.portal);
