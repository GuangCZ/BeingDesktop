...
try {
  ...
  $taskProcess=Start-Process -FilePath (Join-Path $taskRoot 'node_modules\electron\dist\electron.exe') -ArgumentList ('"'+$taskRoot+'"') -WorkingDirectory $taskRoot -WindowStyle Hidden -RedirectStandardOutput (Join-Path $taskProfile 'stdout.log') -RedirectStandardError (Join-Path $taskProfile 'stderr.log') -PassThru
