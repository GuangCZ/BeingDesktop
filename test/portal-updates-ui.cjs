...
if (!process.versions.electron) {
  ...
  async function run() {
    ...
    check('unconfigured-is-not-claimed-uninstalled', await execute("document.getElementById('portal-update-version').textContent==='尚未配置'&&document.getElementById('portal-update-status').textContent.includes('选择 Portal 程序后')"));
    ...
    await click('portal-update-check');
    await waitFor(()=>pendingCheck,'external read IPC');
    const externalRead=pendingCheck;pendingCheck=undefined;externalRead.resolve(structuredClone(fixtureState));
    await domWait("!document.getElementById('portal-update-check').disabled");
    await capture('04-external-portal-720');
    fixtureState.portal.status='stopped';
